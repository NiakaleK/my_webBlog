...
$query=$dtb->prepare('SELECT id_billet,title,content,liens,account.login AS "login",nom,tags,created,genre_billet.id_genre FROM billets INNER JOIN genre_billet ON billets.id_genre=genre_billet.id_genre INNER JOIN account ON billets.id_user=account.id_user WHERE genre_billet.id_genre IN (1,2,3) ORDER BY id_genre DESC LIMIT 3;');

$query->execute();

if($query->rowcount() >0){
	echo'<h2>Les dernières découvertes</h2>
	<div class="posts">';
