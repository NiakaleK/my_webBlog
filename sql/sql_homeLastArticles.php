...
$query=$dtb->prepare('SELECT id_billet, title,content,liens,account.login AS "login",nom,tags,created FROM billets INNER JOIN genre_billet ON billets.id_genre=genre_billet.id_genre INNER JOIN account ON billets.id_user=account.id_user ORDER BY created DESC limit 3');

$query->execute();

if($query->rowcount() >0){
	echo'<h2>Les derniers articles</h2>
	<div class="posts">';
